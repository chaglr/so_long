...
/*   Updated: 2023/06/20 18:48:15 by muhnal           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	one_exit_checker(t_win *d)
{
	...
	{
		write(1, "Error\n", 6);
		write(1, "\nopen_windows_checker.c: Only one EXIT Allowed\n", 48);
		close_frame(d);
	}
}
...
void	validate_single_player(t_win *d)
{
	...
	{
		write(1, "Error\n", 6);
		write(1, "\nopen_windows_checker.c: Only one PLAYER Allowed!\n", 51);
		close_frame(d);
	}
}
...
void	mapsize_check(t_win *d)
{
	...
	while (map[i])
	{
		...
		{
			write(1, "Error\n", 6);
			write(1, "\nopen_windows_checker.c:Line size should be equal\n", 51);
			close_frame(d);
		}
		i++;
	}
